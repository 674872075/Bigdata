...
* C:常见代码块的应用
	...
	* c:静态代码块 
		* 在类中方法外出现，并加上static修饰；用于给类进行初始化，在类加载的时候就执行，并且只执行一次。（如果类中有main方法，执行在main方法之前）
		* 一般用于加载驱动
...
* A:Java中类的继承特点
	* a:Java只支持单继承，**不支持多重继承**。(一个儿子只能有一个爹)
		* 有些语言是支持多继承（C++），格式：extends 类1,类2,...
	* b:Java**支持多层继承**(继承体系)
* B:案例演示
	* Java中类的继承特点
...
* A:this和super都代表什么
	* this:代表当前对象的引用,谁来调用我,我就代表谁（this只和特定的对象关联，而不和类关联，同一个类的不同对象有不同的this）
	* **this 对象，它可以在类里面来引用这个类本身的属性和方法，说白了就是this是对象new出来后才会存在的，而static方法是不需要new出来直接可以调用的。所以在调用static方法并不能保证this已经存在，自然会报错！所以在static方法中只能引用static的变量和方法。**
	* super:代表当前对象父类的引用
* B:**this和super的使用区别**
	* a:调用成员变量
		* this.成员变量 调用本类的成员变量,也可以调用父类的成员变量（前提是本类没有）
		* super.成员变量 调用父类的成员变量
	* b:调用构造方法
		* this(...)	调用本类的构造方法
		* super(...)	调用父类的构造方法
	* c:调用成员方法
		* this.成员方法 调用本类的成员方法,也可以调用父类的方法（前提是本类没有）
...
* A:方法重写的面试题
	...
	* **方法重写**：子类中出现了和父类中方法声明一模一样的方法。与返回值类型有关,返回值是一致(或者是子父类)的（子类重写父类方法时，访问权限不能更低），参数列表一致
	
	* **方法重载**：本类中出现的方法名一样，参数列表不同的方法。与返回值类型无关，与访问修饰符无关。
...
* B:final修饰特点
	* 修饰类，类不能被继承
	* 修饰变量，变量就变成了常量，只能被赋值一次（局部变量也能被修饰）
	* 修饰方法，方法不能被重写
* C:案例演示
	* final修饰特点
