...
- value数组被private修饰，并且没有为这个变量提供任何get和set方法，其他类无法访问
...
BeanFactory是一个工厂，是IOC容器的顶级接口,spring中的工厂，它能够去生产我们的spring bean,并且能够去获取我们的对象
...
>环绕通知（@Around）：  通知方法会将目标方法封装起来， 环绕目标方法执行 
>
>前置通知（@Before）:   通知方法会在目标方法调用之前执行 
>最终通知（@After）: 通知方法会在目标方法调用之后执行 (出现异常仍会执行)
>后置通知（@AfterReturning）:通知方法会在目标方法返回后执行方法结束之前执行 （出现异常不会执行） 
...
​	Spring IOC解决循环依赖需要使用到三级缓存，一级缓存(singletonObjects)存储的是成品对象、二级缓存(earlySingletonObjects)存储的是半成品对象(未完全填充属性值、未完成Spring Bean的初始化操作)、三级缓存(singletonFactories)存储的是ObjectFactory，用来创建代理对象。分阶段存储对象内容，用于解决循环依赖问题
...
**优点:**
1、开箱即用，使用比较简单
2、starter自动配置，简化了配置构建
3、集成很多第三方组件
4、内嵌web容器，部署简单
5、提供了生产级别的监控和健康检查
**缺点:**
1、迭代快
2、封装得比较深，内部原理复杂，易学难精
...
**优点:**
1、职责专一，可以由专门的团队负责专门的服务
2、服务拆分粒度更细，服务独立部署，耦合性降低
3、服务拆分使得依赖库和代码量减少，服务的启动速度更快
4、服务拆分可以快速发布新版本，更加适合敏捷开发
5、服务可以方便的动态按需扩容
6、方便代码复用
**缺点:**
1、架构更加复杂，调用复杂度增加
...
1、**初始定位不同：**SpringCloud定位为微服务架构下的一站式解决方案，生态更加完备；Dubbo 是 SOA 时代的产物，它的关注点主要在于服务的调用和治理
2、**调用方式不同:**  Dubbo底层是使用Netty这样的NIO框架，是基于TCP协议传输的，配合以Hession序列化完成RPC通信;
而SpringCloud一般使用OpenFeign进行服务调用，它是基于Http协议的，相对来说，Http请求会有更大的报文，占的带宽也会更多
...
- Random LoadBalance（加权随机，按照权重的设置随机概率）

- RoundRobinLoadBalance（加权轮询，按照权重设置轮询比率）
...
我们传统的检索方式是通过遍历整篇文章，逐个比对找到对应的关键词位置，而倒排索引是通过分词策略，形成词和文章的关系映射表，这种*词典+映射表的方式就是倒排索引*，有点类似于我们以前使用的新华字典。倒排索引可极大的提高查询效率。
...
1. 会进行分词，分词后建立索引。【比如：对于‘佟永硕’，ik分词器的smart分词会自动将其分成佟、永、硕三个字符进行建立索引，所以单字符搜索可以搜索到，而比如‘永硕’则搜索不到】
2. 支持模糊查询，支持准确查询。
3. 不支持聚合查询

**keyword类型：**

1. 不分词，直接建立索引。【依据此特点，可以使用keyword类型+wildcardQuery（通配查询）实现类似sql的like查询（模糊搜索）】
2. 支持模糊查询，支持准确查询。
3. 支持聚合查询。
...
> 参考资料: https://www.jianshu.com/p/750167f63262