...
![](assert\2019-08-27_09-18-23.png)
...
![](assert\2019-08-27_09-23-23.png)

![](assert\2019-08-27_09-25-07.png)
...
![](assert\2019-08-27_09-38-56.png)
...
-  解决碎片化问题
-  顺序分配内存,简单高效
- 适用于对象存活率低的场景（如果内存中的垃圾对象较多，需要复制的对象就较少，这种情况下适合使用该方式并且效率比较高）

![](assert\2019-08-27_09-42-58.png)

##### 年轻代GC(8:1:1)

![](assert\2019-08-27_10-00-55.png)

1. 在GC开始的时候，对象只会存在于Eden区和名为“From”的Survivor区，Survivor区“To”是空的。
2. 紧接着进行GC，Eden区中所有存活的对象都会被复制到“To”，而在“From”区中，仍存活的对象会根据他们的年龄值来决定去向。年龄达到一定值(年龄阈值，可以通过-XX:MaxTenuringThreshold来设置， 对象晋升老年代的[阈值](https://so.csdn.net/so/search?q=阈值&spm=1001.2101.3001.7020)，**默认值15（并不是绝对的，如果在Survivor空间中相同年龄所有对象大小的综合大于Survivor空间的一半，年龄大于或等于该年龄的对象就可以直接进入老年代）** )的对象会被移动到老年代中，没有达到阈值的对象会被复制到“To”区域。
...
![](assert\2019-08-27_09-46-17.png)
...
![](assert\2019-08-27_09-54-19.png)
...
- Minor GC
  ...
  **GC触发条件：Eden区满了触发Minor GC，这时会把Eden区存活的对象复制到Survivor区，当对象在Survivor区熬过一定次数(年龄阈值[复制的次数]，可以通过-XX:MaxTenuringThreshold来设置,默认15)的Minor GC之后，就会晋升到老年代（当然并不是所有的对象都是这样晋升的到老年代的），当老年代满了，就会报OutofMemoryError异常。**
...
![](assert\2019-08-27_10-10-32.png)

![](assert\2019-08-27_10-20-57.png)
...
![](assert\2019-08-27_10-41-19.png)

##### ParNew收集器(-XX:+UseParNewGC,复制算法)

![](assert\2019-08-27_10-43-26.png)

##### Parallel Scavenge(-XX:+UseParallelGC ,复制算法)

![](assert\2019-08-27_10-46-01.png)
...
![](assert\2019-08-27_10-53-11.png)

##### Parallel Old收集器(-XX : +UseParalleloldGC,标记-整理算法)

![](assert\2019-08-27_10-56-07.png)

##### CMS收集器(-XX:+UseConcMarkSweepGC,标记-清除算法)

![](assert\2019-08-27_10-59-40.png)

![](assert\2019-08-27_11-00-26.png)
...
>2. 第二步，设置堆的最大内存       -Xmx256m    
>3. 第三步，设置最大的停顿时间   -XX:MaxGCPauseMillis=100
>
>G1中提供了三种模式垃圾回收模式，Young GC、Mixed GC 和 Full GC，在不同的条件下被触发
>
>一般来说只有Young GC、Mixed GC特殊情况下会切换到Full GC 

**原理**

G1垃圾收集器相对比其他收集器而言，最大的区别在于它取消了年轻代、老年代的物理划分，取而代之的是将堆划分为若干个区域（Region），这些区域中包含了有逻辑上的年轻代、老年代区域。

这样做的好处就是，我们再也不用单独的空间对每个代进行设置了，不用担心每个代内存是否足够。G1从整理上看是标记-整理算法，局部看是复制算法

![](assert\2019-08-27_11-12-15.png)
...
![](assert\2019-08-27_11-14-20.png)
...
![](assert\2019-08-27_11-14-52.png)
...
Evacuation阶段是全暂停（**STW**）的。该阶段把一部分Region里的活对象拷贝到另一部分Region中，从而实现垃圾的回收清理。



###### **Full GC** 

Full GC是所有G1垃圾回收调优者尽力回避的情况，停止系统程序，单线程回收垃圾，回收对象是整个堆，不再受最长停顿时间约束，一旦出现此情况，意味着应用的响应时间无情的变长。
...
![](assert\2019-08-27_11-07-32.png)

## GC相关的面试题

### Object的finalize方法的作用是否与C++的析构函数作用相同

![](assert\2019-08-27_11-34-42.png)

### Java中的强引用,软引用,弱引用,虚引用有什么用

#### 强引用

![](assert\2019-08-27_11-39-52.png)

#### 软引用

![](assert\2019-08-27_11-42-19.png)

#### 弱引用

![](assert\2019-08-27_11-45-07.png)

#### 虚引用

![](assert\2019-08-27_11-45-55.png)

![](assert\2019-08-27_11-47-26.png)

#### 类层次结构

![](assert\2019-08-27_11-48-29.png)

![](assert\2019-08-27_15-28-48.png)
