...
RDB(快照)持久化 : 保存某个时间点的全量数据快照（xxx.rdb文件是一个紧凑的二进制文件， 整体上可分为三个区域：头信息区、数据区、尾信息区。 ）
...
特点：将所有写命令写入到日志文件中，AOF会将所有的写命令写到AOF文件的末尾，以此来记录数据的变化。记录的不是状态，而是所有的写命令。默认不开启。相当于记录在日志中。
如何开启：在配置文件中，将appendonly修改为yes即可。
...
写入频率：
1.always【谨慎使用】每个redis命令都要写在硬盘中，严重影响redis的速度。
2.everysec【推荐】每一秒执行一次，将多个写命令同步到磁盘中。
3.no【不推荐】由操作系统决定何时同步
修改写入频率：在配置文件中，将appendfsync everysec修改成其他的即可

#### AOF文件的重写

AOF带来的问题：随着使用时间的推移，AOF文件可能会存有很多没用的数据，会使得AOF文件体积庞大。为了压缩AOF文件，redis提供了AOF重写机制。
...
- pipeline 关注的是 RTT时间(一次请求响应的往返时间) 和减少 IO 调用，事务关注的是一致性问题

> 参考: https://www.cnblogs.com/immaxfang/p/16819288.html
...
**跳转重定位**
 ...
 　当客户端向一个节点发出了指令（先计算出key对应槽位，再根据本地缓存的集群槽位配置信息找到槽位对应节点），首先当前节点会计算指令的 key 得到槽位信息，判断计算的槽位是否归当前节点所管理；若槽位不归当前节点管理，这时它会向客户端发送一个特殊的跳转指令携带目标操作的节点地址，告诉客户端去连这个节点去获取数据。客户端收到指令后除了跳转到正确的节点上去操作，还会同步更新纠正本地的槽位映射表缓存，后续所有 key 将使用新的槽位映射表。
...
```
 ...
 1、一但执行 EXEC 开启事务的执行后，无论事务使用执行成功， WARCH 对变量的监控都将被取消。
故当事务执行失败后，需重新执行WATCH命令对变量进行监控，并开启新的事务进行操作。

2、 watch指令类似于乐观锁，在事务提交时，如果watch监控的多个KEY中任何KEY的值已经被其他客户端更改，则使用EXEC执行事务时，事务队列将不会被执行，同时返回Nullmulti-bulk应答以通知调用者事务执行失败。  
...
看门狗自动续期，Watch Dog 机制其实就是一个后台定时任务线程，获取锁成功之后，会将持有锁的线程放入到一个 RedissonLock.EXPIRATION_RENEWAL_MAP
